...
function isBalanced(treeRoot, leftCount = 0, rightCount = 0) {
  //set two counters
  let leftCounter = leftCount;
  let rightCounter = rightCount;
  // Determine if the tree is superbalanced
  //check if treeRoot.left is null
  if(treeRoot.left === null || treeRoot.right === null){
    ...
    if(treeRoot.left){
      leftCounter++
      isBalanced(treeRoot.left, leftCounter, rightCounter)
    }
  //check if treeRoot.right is null
    if(treeRoot.right){
    //if null return count
      rightCount++
      isBalanced(treeRoot.right, leftCounter, rightCounter)
    }
  
  //if leftCount - rightCount <= 1
  if(Math.max(leftCounter, rightCounter) - Math.min(leftCounter, rightCounter) <= 1){
    //return true
    return true
  }
